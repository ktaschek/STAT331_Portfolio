...
# REVISION:
# modified to be more efficient, using the across function
...
# NEW:
# Based on the above lab label
# create a new variable using pmap_dbl()
...
teacher_evals |>
  ...
  # refernce for pmap_dbl: https://purrr.tidyverse.org/reference/pmap.html
  mutate(
    adjusted_SET_score = pmap_dbl(
      list(SET_score_avg, percent_failed_cur, no_participants),
      ~ ifelse(..3 >= 10, ..1 * (1 - ..2 / 100), -1)
    )
  )|>
  filter(
    adjusted_SET_score != -1
  )
